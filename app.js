var animation_length=7000; //7s
var initial_timeout=1000; //1s
...
var data = codes.split(",").sort().map(function (code) {
        return {code: code,
                value: 0}});
...
function sizeChart() {
  if (window.innerHeight>window.innerWidth) {
    // alert('portrait');
    mapChart.setSize((window.innerHeight-120)*813/388.29, (window.innerHeight-120), true);
    mapChart.xAxis[0].setExtremes(4558.68,4558.68+3000, false)
    mapChart.yAxis[0].setExtremes(-8200.36-750, -8200.36+1250)
  }
  else {
    // alert('landscape');
    mapChart.setSize((window.innerWidth), (window.innerHeight-70), true);
  }
}
sizeChart();

// mapChart.series[0].data.filter(point => point.code == "IE")[0].zoomTo();
...
customElements.define('modal-dropdown-page', class extends HTMLElement {
  connectedCallback() {
    //get EU, SA, NA etc. arrays here I guess
    var html_string="";
    var html_string=`
<ion-header>
    <ion-toolbar>
...
var unvisited=mapChart.series[0].data.filter(point => point.value == 0);
var visited=mapChart.series[0].data.filter(point => point.value == 1);
//visited dropdown
for (var i=0; i<visited.length; i++) {
  html_string+=`<ion-item class="dropdown_list_item" onClick=`+"updatemap('"+visited[i].code+`')>
    <ion-label>`
...
for (var i=0; i<unvisited.length; i++) {
  html_string+=`<ion-item class="dropdown_list_item" onClick=`+"updatemap('"+unvisited[i].code+`')>
    <ion-label>`
...
this.innerHTML = html_string;
  ...
  const items = Array.from(document.getElementsByClassName('dropdown_list_item'));
...
customElements.define('modal-animation-page', class extends HTMLElement {
  connectedCallback() {
    var visited=mapChart.series[0].data.filter(point => point.value == 1);
    var html_string=""
    html_string=`
...
for (var i=0; i<visited.length; i++) {
  html_string+=`
    <ion-item class="font_up">
      <ion-label class='animate_labels'>`+visited[i].name+" ("+visited[i].code+`)</ion-label>
      <ion-datetime id="test" value="`
  var pts=animation_base_data.filter(point => point[0] == visited[i].code);
  if (pts.length>0) {
...
function closeAnimateModal() {
  ...
  for (var i = 0; i < dates.length; i++) {
    ...
    if (date.length==1) {
      ...
      animation_base_data.push([vals[i].textContent.split('(')[1].substring(0,2), date])
    }
  }
...
function playAnimation(data, index) {
  ...
  if (index<data.length-1){
      setTimeout(playAnimation, data[index+1][1]-data[index][1] , data, index+1)
  }
}
...
function getAnimationData(initial_animation_data) {
  ...
  for(let i=0; i<initial_animation_data.length; i++) {
    var months=initial_animation_data[i][1][0]*12+initial_animation_data[i][1][1]*1;
    animation_data.push([initial_animation_data[i][0],months, initial_animation_data[i][1]]);
    if (min>months) {
      min=months;
    }
  }
  animation_data=animation_data.sort(function(a, b){return a[1] - b[1]});
  ...
  var normalised_data=offset_data.map( function(point) {
    return [point[0], point[1]*animation_length/offset_data[offset_data.length-1][1], point[2]];
  } );
